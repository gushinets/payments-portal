...
test("Playwright runtime env loader shares harness secrets", async () => {
  ...
  await writeFile(
    ...
    [
      "SMTP_PASSWORD=secret_from_runtime",
  ...
  assert.equal(targetEnv.SMTP_PASSWORD, "secret_from_runtime");
...
test("Playwright runtime env loader preserves explicit process overrides", async () => {
  ...
  const targetEnv = { SMTP_PASSWORD: "secret_from_process" };
  loadRuntimeEnv(repositoryRoot, targetEnv);

  assert.equal(targetEnv.SMTP_PASSWORD, "secret_from_process");
});
