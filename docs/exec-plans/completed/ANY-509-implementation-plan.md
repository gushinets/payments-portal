...
- Required stable constraints: `UNIQUE(external_billing_account_id, user_id)` and globally `UNIQUE(billing_customer_key)` across all retained customer slots.
- `billing_customer_key` is immutable, opaque, non-PII, permanently reserved locally, and never reused for another customer slot, including after configured billing-account scope changes. The customer-slot allocation record must not be hard-deleted in a way that permits key reuse; retention/non-reuse is part of the identity invariant, not merely a transient uniqueness check.
...
- The primary link is a composite `(primary_subscription_id, user_id, product_id)` FK, so the database cannot select a subscription from another user/product scope.
- The canonical normal-purchase lock order is `product access scope -> customer slot`: acquire/get-and-lock this row first, then acquire/create and lock the canonical `(external_billing_account_id, user_id)` customer row in the same short transaction. No purchase path may reverse those locks.
...
- Persists user, canonical external-customer slot, billing-account scope, Kernel `product_id`, external `billing_offer_id`, mapping revision, accepted commercial fingerprint, client idempotency identity, orchestration state/timestamps, optional proven linked subscription, and one immutable typed `accepted_snapshot`.
...
- Business serialization is `(user_id, product_id)` through `billing_product_access_scopes`. Public request identity is locked as `(external_billing_account_id, user_id, client_idempotency_key)` with a Step-4 unique constraint; Step 7 implements exact replay and rejects same-identity/different-request reuse.
- Composite FKs make the customer/account/user scope agree and make `mapping_revision_id` belong to the purchase's exact account/offer.
...
- After the normal purchase flow has locked its product scope, customer creation is serialized across products by insert with exact conflict target `(external_billing_account_id, user_id)` plus `FOR UPDATE` on the canonical customer slot. A generated `billing_customer_key` is only an insertion candidate: a flow that loses the account/user insert race discards its candidate without comparison and reuses the winner's immutable persisted key. A global `billing_customer_key` collision with any retained slot, including one under another billing-account scope, is local allocation failure/reallocation before external effect, never reuse of that slot or `identity_conflict`. A Step-4 partial unique constraint permits at most one unresolved `customer` create operation per customer slot, using `resolved_at` rather than the later-owned operation-state vocabulary. Intent commits before provider I/O; ambiguous outcomes remain unresolved and are never blind-retried.
- Different-product purchases may hold different product-scope rows and contend on the same customer row; the waiter then reuses the canonical customer slot and unresolved customer-create operation. Customer-only recovery may lock customer/create-operation state but must not subsequently acquire a product-scope lock.
...
- Uses an internal Portal UUID primary key and stores billing-account/customer/user scope, Kernel `product_id`, pinned purchase/mapping provenance, opaque external subscription/agreement identifiers, normalized provider-neutral lifecycle/financial/commercial access statuses, `last_authoritative_read_at`, and `projection_valid_until`.
- Composite customer and purchase FKs make account/user/product/mapping provenance agree. The repeated subscription user is not independent truth; it exists so the primary-subscription composite FK can enforce exact user/product ownership. Purchased allowances reference the subscription together with `product_id`.
- Stores the PostgreSQL reconciliation lease owner/expiry and monotonic fencing token needed by later reconciliation, plus an optional reference to the latest durable normalized `authoritative_subscription_read` observation that produced the current projection. The composite FK enforces the same subscription; Step 8 enforces the observation kind and projection causality without duplicating the kind discriminator.
- Reconciliation lease owner/expiry and work-item lease owner/expiry each have a Step-4 paired-nullability check; half-populated leases are impossible.
...
- `commercial_access_status = eligible` requires complete material-term match, a valid linked PurchaseIntent with immutable accepted commercial/legal evidence and pinned mapping provenance, valid origin rules, and no access conflict. `purchase_intent_id` remains nullable so a proven external subscription discovered outside a valid Portal purchase flow can be persisted and reconciled, but such a row remains commercially ineligible, contributes no paid product grant or purchased allowance, and may enter conflict/manual review. Authoritative provider lifecycle/financial facts, primary selection, or manual-review resolution cannot manufacture or bypass the missing Portal purchase/mapping/legal provenance. The same predicate applies to unmetered offers even when no metric allowance exists.
...
- `resulting_access_revision` is the sole intentionally late-bindable audit field, not a physical FK to the mutable current `paid_access_states.access_revision`. An observation may commit with it `NULL`; Step 9 may later update it from `NULL` to one positive revision `N` exactly once, only in the same transaction that commits effective state revision `N`, the resulting semantic access state, and the corresponding invalidation/outbox update. Once populated it is immutable, `N` -> `M` is forbidden, and later revisions never rewrite it. Canonical user/access-scope agreement is enforced by the existing scope/Application transaction invariants, and the scope/revision index is audit lookup only.
...
- Observation-level `subscription_id` and `purchase_intent_id` are mutually exclusive. When a subscription is referenced, purchase provenance is derived through its immutable nullable `external_subscriptions.purchase_intent_id`; a direct observation purchase reference is allowed only without a subscription reference, so unlinked discovered subscriptions remain valid and contradictory historical subscription/purchase pairs are unrepresentable.
- Observation identity, kind, scope references, normalized evidence, classifications, timestamps, and evidence meaning are immutable after insertion. `resulting_access_revision` is the sole exception: it may transition from `NULL` to the one positive revision committed by the causative semantic access transition, at most once and in that same transaction; after population it is immutable. Rows remain queryable while referenced by current/historical subscription, purchase, access-revision, conflict/manual-review, or operational audit requirements.
...
- Materialization requires the source subscription to be linked to the applicable `PurchaseIntent` and its pinned mapping revision, with source component, resolved product/metric bindings, and fixed integer quantity exactly matching the immutable accepted snapshot. Unlinked discovered subscriptions and missing or contradictory provenance produce no allowance or paid metered access and fail closed through the existing conflict/manual-review path where appropriate; runtime never reconstructs quantity from mutable provider state.
- When a new `allowance_id` is first committed, Step 9 derives and freezes `period_start = raw/source period_start + clock_skew_budget` and `period_end = raw/source period_end - clock_skew_budget` using the deployment budget then in force. A collapsed conservative interval is unusable and fails closed rather than being widened. Later budget changes apply only to future allowance IDs; any reinterpretation that would change an existing tuple is conflict/fail-closed behavior and must not mutate the tuple or reset Kernel usage identity.
- MVP does not stack multiple effective allowances for one `(product_id, metric_key)`. Conflicting effective candidates are omitted for that metric without summing or selection by row order, recency, amount, worker order, or last-write-wins; retain conflict/manual-review evidence, preserve unrelated metrics/products, and never invent a replacement `allowance_id` or reset Kernel usage. Provider-cycle identity/uniqueness remains `PHASE_0_GATED`.
...
- Stores `pending_revision > 0`, `delivered_revision >= 0`, and `pending_revision >= delivered_revision`, plus attempt count, `next_attempt_at`, safe last-error classification, and timestamps. Revision `0` is implicit and has no durable invalidation; the first material paid-access change atomically creates committed revision `1` and pending invalidation `1`. Local acknowledgement is monotonic and must never move `delivered_revision` backward or beyond the revision actually sent.
...
- A resolution may bind a proven object or release a scope after audited proof, but cannot directly set entitlement active, invent allowance quantity, silently rebind historical mapping/purchase semantics, or bypass missing valid pinned purchase/mapping/legal-acceptance provenance.
...
- one customer slot is race-safe per `(external_billing_account_id, user_id)` and `billing_customer_key` is globally unique and cannot be reused across retained slots or billing-account scope changes;
- concurrent purchases for different products use `product access scope -> customer slot`, converge on that customer slot, discard any losing insertion candidate key, reuse the persisted canonical key, and cannot establish two unresolved customer-create operations;
- one product scope row serializes `(user_id, product_id)` purchase/primary decisions;
- duplicate client retries are physically unique on `(external_billing_account_id, user_id, client_idempotency_key)` and do not create a second business flow once Step 7 implements replay/conflict semantics;
- cross-table mapping/customer/purchase/primary/allowance and scope-bearing observation references reject mismatched account/user/product/offer scope;
- observations cannot pair a subscription with an independently contradictory purchase; subscription-referenced purchase provenance is derived through `external_subscriptions.purchase_intent_id`;
...
- `billing_state_observations` survive restart and later projection replacement, preserve normalized authoritative-read/discovery/primary/deterministic-boundary evidence, and retain `resulting_access_revision = N` for revision-N causative evidence after current state advances to `N+1`, without retaining raw provider/payment history; Step 9 commits revision, effective state, causative linkage, and outbox consistently;
- every `external_subscriptions.latest_observation_id` references the same subscription and is an `authoritative_subscription_read` that produced the current projection;
- a discovered subscription with `purchase_intent_id = NULL` can be persisted and reconciled but remains commercially ineligible and produces neither a paid product grant nor a purchased allowance, including for an unmetered offer; becoming primary or resolving manual review cannot bypass missing valid pinned purchase/mapping/legal provenance, and authoritative provider state alone cannot manufacture it;
- once a valid linked PurchaseIntent with immutable accepted commercial/legal evidence and pinned mapping provenance exists and every other accepted commercial predicate is satisfied, normal Step-9 paid-access derivation may produce the applicable paid product grant and, for metered offers, a purchased allowance whose component/product/metric/quantity matches the accepted snapshot;
- a materialized allowance freezes effective bounds using the `clock_skew_budget` in force at first commit; later budget changes affect only future allowance IDs, collapsed intervals fail closed, and reinterpretation cannot mutate an existing tuple;
- conflicting duplicate effective allowance candidates for one `(product_id, metric_key)` are omitted for that metric without stacking, heuristic selection, replacement identity, or usage reset; conflict/manual-review evidence remains while unrelated metrics/products remain eligible;
...
- insertion of `pending_revision = 0` is rejected; revision zero has no durable invalidation, and the first material transition atomically creates paid-access revision `1` plus pending invalidation `1` while `delivered_revision` may remain `0`;
...
- subscription and work-item lease owner/expiry fields are both null or both non-null;
...
5. Lock `external_billing_customers` customer-slot identity on `UNIQUE(external_billing_account_id, user_id)`, global key reservation on `UNIQUE(billing_customer_key)`, and permanent key non-reuse across retained slots and configured billing-account scope changes. Do not treat email/phone/name as identity. Purchases and subscriptions use composite FKs to this canonical account/user customer scope.
6. Lock `(user_id, product_id)` serialization through `billing_product_access_scopes` and the complete observe-before-lock first-primary algorithm from the locked storage contract, including durable normalized observation before the short product-scope transaction, union re-read under lock, deterministic zero/one/many decision rules, and no implicit promotion/replacement heuristics. Enforce the primary link with an exact user/product composite FK. Do not introduce a separate primary-decision generation subsystem.
7. Lock one unified `external_create_operations` model for customer/agreement/subscription create uncertainty. Preserve durable UNKNOWN, fixed two-hour escalation, no blind create retry, and no zero-match absence inference. For normal purchases, require the single lock order `product access scope -> customer slot`: acquire/get-and-lock the product row first; then insert the customer slot with exact conflict target `(external_billing_account_id, user_id)`, select it `FOR UPDATE`, and apply the Step-4 partial unique guard for at most one unresolved customer-create operation. A losing account/user insert discards its candidate key and reuses the persisted canonical key; a billing-customer-key collision with another slot is only local allocation failure/reallocation before external effect and never `identity_conflict`. Customer-only recovery must not create a reverse lock path. No provider deduplication assumption is allowed.
8. Lock the proven-subscription-only rule for `external_subscriptions`, projection freshness fields, reconciliation fencing storage, historical-row retention needed for identity/recovery/audit, and the accepted normalized vocabularies: lifecycle `active|inactive|ended`, financial access `allowed|blocked`, commercial access `eligible|ineligible`. Keep `purchase_intent_id` nullable for discovered subscriptions, but require complete material-term match, a valid linked PurchaseIntent with immutable accepted commercial/legal evidence and pinned mapping provenance, valid origin rules, and no access conflict before commercial eligibility or any paid grant/allowance derivation. Primary selection, authoritative provider state, and manual-review resolution cannot bypass that predicate, including for unmetered offers. Composite FKs must enforce linked purchase customer/account/user/product/mapping agreement, allowance product agreement, and paired lease nullability. Do not enforce the unproven LBX canonical subscription/agreement uniqueness constraints before Phase 0.
9. Lock `billing_state_observations` as the append-only minimized evidence boundary for authoritative subscription reads, target-product discovery, first-primary selection, and deterministic access-reducing boundaries. Require typed normalized payloads, immutable evidence meaning, safe references to the affected local state, and optional `resulting_access_revision`; forbid raw provider payload archives, payment ledgers, and a second business state machine. Make subscription and direct-purchase references mutually exclusive so subscription purchase provenance derives only through `external_subscriptions.purchase_intent_id`; require Step 8 to validate that each subscription `latest_observation_id` is the same-subscription `authoritative_subscription_read` that produced its current projection.
10. Lock purchased allowance integer quantity, `quantity >= 0`, valid effective half-open interval with `period_start < period_end`, absence of runtime `remaining`, and the immutable effective tuple derived at first commit using the then-current `clock_skew_budget`. Later budget changes affect only future allowance IDs; collapsed intervals and reinterpretations that would mutate an existing tuple fail closed. Lock metric-local duplicate handling: do not stack or heuristically select conflicting effective candidates for one `(product_id, metric_key)`; omit that metric, retain conflict/manual-review evidence, preserve unrelated facts, and do not invent a replacement identity or reset usage. Mark provider cycle identity/uniqueness as Phase 0 gated.
11. Lock `paid_access_states` as the current committed provider-neutral semantic snapshot with implicit revision zero when absent. Lock semantic scope as `(tenant_id, region, canonical Portal user_id)`, but explicitly delegate only the physical representation of tenant/region in the table/FK key to the `ANY-504` Step 3 -> Step 4 handoff. Any semantic change must increment revision and update the invalidation outbox atomically; GET/read paths never create or mutate access state. Require `pending_revision > 0`, `delivered_revision >= 0`, and `pending_revision >= delivered_revision`; revision zero has no outbox row, and the first material transition atomically creates revision/invalidation `1`.
...
13. Lock minimal/redacted webhook evidence and manual-review semantics. Neither webhook payload nor manual review directly grants access, and manual-review resolution cannot bypass missing immutable purchase/mapping/legal-acceptance provenance. Add the durable audit/evidence-source matrix from the locked research section and map successful authoritative-read/reconciliation, first-primary, and deterministic-boundary proof to `billing_state_observations`; current mutable projections or logs alone are insufficient.
14. Lock projection/mapping identity semantics needed by future steps: `product_id`/`metric_key` remain Kernel identifiers without Portal catalog FKs; external offer/component IDs remain opaque rather than Portal catalog FKs; manifest/catalog admission freshness is 24h for new mapping/purchase only; `commercial_mapping_revisions` have explicit `(external_billing_account_id, billing_offer_id)` scope, immutable monotonic revision-number current selection, pin manifest version + catalog version/digest, and are referenced by purchase through that exact account/offer scope.
...
- Provider-independent concurrency/idempotency invariants are defined: customer-slot uniqueness and cross-product customer-create serialization, durable conflict state, product-scope serialization with the observe-before-lock first-primary algorithm, locked `(account, user, client key)` purchase idempotency identity, durable UNKNOWN, scope-safe composite FKs, paired lease constraints, reconciliation fencing, immutable purchase/mapping/allowance state, revision + outbox atomicity, and one shared durable work substrate.
