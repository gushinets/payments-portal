...
Do NOT run tests, pytest, linters, formatters, type checkers, generators, architecture checks, Alembic commands, `npm run check`, `npm run check:fast`, `npm run test:api`, or any other automated verification command.

Do not automatically reformat unrelated files.

The human will run all verification manually after reviewing the diff.
...
```bash
pytest apps/api/tests/test_billing_lifecycle.py -k "order_plan_scope_mismatch or missing_order_plan or paid_orders_create_distinct_entitlements"
npm run test:api
npm run check:fast
```
