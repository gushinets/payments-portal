...
- `billing_state_observations`: reject DELETE and reject UPDATE of every field except `resulting_access_revision` and the FK-driven cleanup transition `work_item_id` non-null -> `NULL`; `work_item_id` reassignment and `NULL` -> non-null remain rejected. `resulting_access_revision` may change at most once from `NULL` to a non-null value, while positivity and semantic/causal binding remain ANY-504 Step 9 runtime. A populated value cannot be replaced or cleared;
