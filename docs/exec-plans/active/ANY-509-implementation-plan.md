...
- a linked subscription with component/product/metric/quantity matching its immutable accepted purchase snapshot can materialize a purchased allowance; an unlinked discovered subscription cannot, mismatched provenance fails closed, later mapping/catalog/provider changes cannot mutate accepted quantity/provenance, and runtime `remaining` remains absent from Portal;
...
5. Lock `external_billing_customers` local uniqueness on `(external_billing_account_id, user_id)` and `(external_billing_account_id, billing_customer_key)` and permanent key non-reuse. Do not treat email/phone/name as identity. Purchases and subscriptions use composite FKs to this canonical account/user customer scope.
...
8. Lock the proven-subscription-only rule for `external_subscriptions`, projection freshness fields, reconciliation fencing storage, historical-row retention needed for identity/recovery/audit, and the accepted normalized vocabularies: lifecycle `active|inactive|ended`, financial access `allowed|blocked`, commercial access `eligible|ineligible`. Composite FKs must enforce linked purchase customer/account/user/product/mapping agreement, allowance product agreement, and paired lease nullability. Do not enforce the unproven LBX canonical subscription/agreement uniqueness constraints before Phase 0.
